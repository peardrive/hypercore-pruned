...
class Peer {
  ...
  async _getProof(batch, msg, pushing) {
    ...
    if (msg.block) {
      ...
      // Pruned mode: if block missing but we have a restore callback, use it
      if (!pushing && msg.fork === this.core.state.fork && !this.core.bitfield.get(index)) {
        if (this.core.onBlockMissing) {
          try {
            await this.core.onBlockMissing(index, this.core)
            // After restore, the bitfield should be updated - re-check
            if (!this.core.bitfield.get(index)) {
              return new ProofRequest(msg, null, null, null)
            }
          } catch (err) {
            // Restore failed, return not available
            return new ProofRequest(msg, null, null, null)
          }
        } else {
          return new ProofRequest(msg, null, null, null)
        }
      } else if (!pushing && msg.fork !== this.core.state.fork) {
        return new ProofRequest(msg, null, null, null)
      }
